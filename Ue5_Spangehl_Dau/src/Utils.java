public class Utils {
	...
	public static int[] fetchRGBValues(int argb) {
		return new int[] { ((argb >> 16) & 0xff), ((argb >> 8) & 0xff),
				(argb & 0xff) };
	}
	...
	public static int[] capValues(int[] rgb) {
		return new int[] { Utils.capValue(rgb[0]), Utils.capValue(rgb[1]),
				Utils.capValue(rgb[2]) };
	}
	 ...
	 */
	public static int[] getKernel(int[] pixels, int width, int height,
		...
		for (int j = 0; j < size; j++) { // Walk the rows (y)
			...
			for (int i = 0; i < size; i++) { // Walk the columns (x)
				...
				kernel[Utils.pixelPos(i, j, size)] = pixels[Utils.pixelPos(
						curPosX, curPosY, width)];
			}
		}
	...
	public static void dilate(int[] srcPixels, int[] dstPixels, int width,
			int height, float radius, int x, int y) {
		...
		if (pixel == Utils.ARGB_BLACK) {
			...
			for (int j = 0; j < size; j++) { // Walk the rows (y)
				...
				for (int i = 0; i < size; i++) { // Walk the columns (x)
					...
					if (distance <= radius) {
						if (curPosX >= 0 && curPosY >= 0 && curPosY < height
								&& curPosX < width) {
	 ...
	 * @return
	 */
	public static double histogramVariance(int[] histogram, double mean,
			int total) {
		double variance = 0.0;
		for (int i = 0; i < histogram.length; ++i) {
	...
	public static void autoCorrect(int[] pixels, int[] histogram, int min,
			int max) {
		int newSteps = -(min - max) / histogram.length; // errechenet abstand
														// neuer balken
